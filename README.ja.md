...
> [!NOTE]
> UI は English・简体中文・日本語に対応し、システム言語に自動で合わせます。トップバーからいつでも切り替えられます。
...
| キー | プロバイダーごとに 2 通り。**環境変数**（推奨）：設定には変数名だけを保存し、転送時に読み取ります。**アプリで入力**：`~/.orrery/proxy.json` に**平文**で保存します（macOS/Linux では本人のみ読み書きできる `0600`）。キーはログに出さず、状態一覧には設定済みかどうかだけを表示します。 |
...
- Orrery 自身のファイルは `~/.orrery/` にあります：ルーティング設定（**アプリで入力した API キーを平文で含みます**）、インデックスのバックアップ、そして `index.json`（セッションの解析結果＝タイトル・パス・トークン数のキャッシュ。再起動を速くするためのもので、削除しても次回スキャンで作り直されます）。
