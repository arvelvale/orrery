...
把本机所有 Claude Code、Kimi Code 会话收进一个窗口：token、磁盘占用、项目、子 agent 一眼看清。数据不出本机。
...
<p>
  ...
  <img alt="DSH" src="https://img.shields.io/badge/DSH-规划中-9AA8B8?style=flat-square" />
  <img alt="MiMoCode" src="https://img.shields.io/badge/MiMoCode-规划中-9AA8B8?style=flat-square" />
</p>
...
| 在终端恢复会话 | ⏳ | 目前只打开会话目录 |
| DSH / MiMoCode 适配器 | ⏳ | 规划中 |

<img src=".github/assets/screenshot-status.zh-CN.png" alt="Openplane 状态页与存储统计" width="100%" />

## token 是怎么算的

两个工具都按 API 调用记录用量，但都不能直接相加：

| | Claude Code | Kimi Code |
|---|---|---|
| 数据来源 | assistant 行的 `message.usage` | `agents/*/wire.jsonl` 里的 `usage.record` 事件 |
| 坑 | 流式输出按内容块拆成多行，每行重复同一份用量（某会话 4,034 行实为 1,558 次调用） | `usageScope: "session"` 是上下文压缩的独立调用，不是汇总 |
| Openplane 的处理 | 按 `message.id` 去重，保留最后一行 | 每条记录计一次 |

会话总量 = 主 agent + 全部子 agent。在同一进程区间内和 Claude Code 自己的 `cost-state` 记账对比，输入、缓存读、输出三项完全一致。
