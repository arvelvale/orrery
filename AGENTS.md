...
- 解析索引：`~/.openplane/index.json` 落盘，冷启动 188 个会话 5.5s → 0.12s
...
10. **删除功能是破坏性操作**：改 `cleanup.rs` 前先读模块顶部的安全约束；验证只在沙盒做（调试构建设 `OPENPLANE_HOME` 指向沙盒主目录），不对真实会话调用 `delete_sessions`；前后对真实目录做快照比对。
