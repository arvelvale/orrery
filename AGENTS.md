...
- 真实扫描：Claude Code、Kimi Code、DSH（DeepSeek）、Codex、OpenCode、Z Code、Antigravity 七家。Z Code（`zcode.rs`，只读，用量以 `model_usage` 为准、逐行按 `computed_total_tokens` 判断缓存是否已含在 input 里）；Antigravity（`antigravity.rs`，只读，每个对话一个 SQLite，用量从 `gen_metadata` 的 protobuf 按字段号解出，cache_read 与 input 分开记）；OpenCode 删除只经官方 `opencode session delete`，回收站模式先 `opencode export` 到 `~/.orrery/exports/`。另有通用适配器 `custom.rs`，读 `~/.orrery/harnesses.json` 登记的 OpenCode 系工具（只读、不支持删除与恢复）。**不要把未公开工具的目录名/表结构写进仓库**，那属于用户本地配置
